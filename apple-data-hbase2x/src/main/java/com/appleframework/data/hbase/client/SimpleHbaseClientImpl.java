...
public class SimpleHbaseClientImpl extends SimpleHbaseClientBase {
    ...
    public <T> boolean updateObjectWithVersion(RowKey rowKey, T t, Object oldVersion) {
        ...
        try {
            result = table.checkAndPut(rowKey.toBytes(), versionedColumnInfo.familyBytes, versionedColumnInfo.qualifierBytes, oldValueOfVersion, put);
        } catch (IOException e) {
    ...
    public void put(String hql) {
        ...
        for (int i = 0; i < hbaseColumnSchemaList.size(); i++) {
            ...
            if (ts == null) {
                ...
                put.addColumn(hbaseColumnSchema.getFamilyBytes(), hbaseColumnSchema.getQualifierBytes(), data);
            } else {
                //put.add(hbaseColumnSchema.getFamilyBytes(), hbaseColumnSchema.getQualifierBytes(), ts.getTime(), data);
                put.addColumn(hbaseColumnSchema.getFamilyBytes(), hbaseColumnSchema.getQualifierBytes(), ts.getTime(), data);
            }
        }
            ...
            if(isDeleteColumn) {
            	for (ColumnInfo columnInfo : columnInfoList) {
                    if (deleteRequest.getTimestamp() == null) {
                        ...
                        delete.addColumn(columnInfo.familyBytes, columnInfo.qualifierBytes, deleteRequest.getTimestamp().longValue());
                    }
                }
            }
            deletes.add(delete);
                    ...
                    if (columnInfoList != null) {
                        for (ColumnInfo columnInfo : columnInfoList) {
                            if (ts == null) {
                                //delete all versions.
                                //delete.deleteColumns(columnInfo.familyBytes, columnInfo.qualifierBytes);
                                delete.addColumns(columnInfo.familyBytes, columnInfo.qualifierBytes);
                            } else {
                                //delete specified version.
                                //delete.deleteColumn(columnInfo.familyBytes, columnInfo.qualifierBytes, ts.getTime());
                                delete.addColumns(columnInfo.familyBytes, columnInfo.qualifierBytes, ts.getTime());
                            }
                        }
                    }
                    ...
                    if (hbaseColumnSchemaList != null) {
                        for (HBaseColumnSchema hbaseColumnSchema : hbaseColumnSchemaList) {
                            if (ts == null) {
                                ...
                                delete.addColumns(hbaseColumnSchema.getFamilyBytes(), hbaseColumnSchema.getQualifierBytes());
                            } else {
                                //delete specified version.
                                //delete.deleteColumn(hbaseColumnSchema.getFamilyBytes(), hbaseColumnSchema.getQualifierBytes(), ts.getTime());
                                delete.addColumns(hbaseColumnSchema.getFamilyBytes(), hbaseColumnSchema.getQualifierBytes(), ts.getTime());
                            }
                        }
                    }
