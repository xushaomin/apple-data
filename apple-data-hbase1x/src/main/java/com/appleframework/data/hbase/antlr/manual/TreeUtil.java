...
public class TreeUtil {
    ...
    public static ProgContext parseProgContext(String hql) {
		...
		try {
			...
			SimpleHbaseStatementsLexer lexer = new SimpleHbaseStatementsLexer(input);
    ...
    public static String parseTableName(ProgContext progContext) {
        ...
        if (progContext instanceof InsertHqlClContext) {
            return InsertHqlClContext.class.cast(progContext).inserthqlc().tablename().TEXT().getText();
        }
        if (progContext instanceof SelectHqlClContext) {
            return SelectHqlClContext.class.cast(progContext).selecthqlc().tablename().TEXT().getText();
        }
        if (progContext instanceof DeleteHqlClContext) {
            return DeleteHqlClContext.class.cast(progContext).deletehqlc().tablename().TEXT().getText();
        }
